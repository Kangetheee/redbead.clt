...
import { ArrowLeft, Save, Eye, Code, Plus, X, HelpCircle } from "lucide-react";
...
This approval link will expire on {{expiryDate}}.`,
  ...
  const fetchTemplate = async () => {
    ...
    try {
      ...
      form.reset({
        ...
        textContent: mockTemplate.textContent || "",
  ...
  const onSubmit = async (data: EmailTemplateFormData) => {
    ...
    try {
      ...
      router.push(`/admin/email-templates/${params.id}`);
    } catch (error) {
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Modify the email template for automated communications
          </p>
        </div>
        {template.isSystem && (
